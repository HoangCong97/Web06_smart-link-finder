...
- `[x]` **Tính điểm ưu tiên hiển thị động (Smart Score) & nâng cấp hiển thị lượt xem** - *Hoàn thành ngày 11/06/2026*:
  * Thiết kế và triển khai hàm `calculatePriorityScore` để tính điểm hiển thị động: Ghim (+10,000đ), click 3 ngày (+15đ), click 30 ngày (+2đ), độ mới (+48đ kịch khung), quá hạn phạt lũy tiến tối đa -240đ.
  * Nâng cấp route `GET /api/links` và `POST /api/search` trong [server.js](file:///c:/Workspace/Web06_smart-link-finder/backend/server.js) để tính và trả về điểm số ưu tiên, đồng thời tự động sắp xếp kết quả.
  * Hiển thị điểm số ưu tiên trực quan dưới tên miền trên [LinkCard.jsx](file:///c:/Workspace/Web06_smart-link-finder/frontend/src/components/LinkCard.jsx) cho tài khoản Admin/Manager, hỗ trợ tự động tái sắp xếp trên frontend trong [App.jsx](file:///c:/Workspace/Web06_smart-link-finder/frontend/src/App.jsx).
- `[x]` **Tạo hiệu ứng viền phát sáng gradient (Glow Pulse) cho các liên kết gấp & hot** - *Hoàn thành ngày 11/06/2026*:
  * Thiết lập keyframe animations `@keyframes glow-pulse-urgent-anim` (màu đỏ cam) và `@keyframes glow-pulse-hot-anim` (màu xanh ngọc/tím) trong [App.css](file:///c:/Workspace/Web06_smart-link-finder/frontend/src/App.css).
  * Cập nhật component [LinkCard.jsx](file:///c:/Workspace/Web06_smart-link-finder/frontend/src/components/LinkCard.jsx) để xác định liên kết gấp (hạn chót còn lại dưới 24h) và liên kết hot (lượt click >= 10) để áp dụng các lớp CSS tương ứng.
  * Mở rộng hiển thị số lượt truy cập (Views Badge) cho cả tài khoản Khách (Guest) và thay thế biểu tượng mặc định bằng icon `Flame` khi liên kết đạt trạng thái Hot.
- `[x]` **Tối ưu lazy loading và conditional mounting cho các Modal** - *Hoàn thành ngày 11/06/2026*:
  * Chuyển đổi các import tĩnh của `LoginModal`, `AdminDashboardModal`, `EditLinkModal`, `ChangePasswordModal`, và `LinkDetailModal` sang import động bằng `React.lazy()` trong [App.jsx](file:///c:/Workspace/Web06_smart-link-finder/frontend/src/App.jsx).
  * Sử dụng `<Suspense fallback={null}>` bao bọc các modal lazy-loaded để hiển thị giao diện mượt mà trong quá trình nạp.
  * Áp dụng cơ chế Conditional Mounting (chỉ mount component khi trạng thái modal mở, ví dụ `{isOpen && <Modal />}`) để tối ưu hóa hiệu năng render của React và giải phóng bộ nhớ khi modal đóng.
