...
- `[x]` **Tắt tính năng zoom trên điện thoại** - *Hoàn thành ngày 11/06/2026*:
  * Cấu hình thẻ meta viewport `maximum-scale=1.0, user-scalable=no` trong [index.html](file:///c:/Workspace/Web06_smart-link-finder/frontend/index.html).
  * Thêm thuộc tính `touch-action: manipulation` vào [index.css](file:///c:/Workspace/Web06_smart-link-finder/frontend/src/index.css) để vô hiệu hóa cử chỉ nhấn đúp để phóng to (double-tap to zoom).
